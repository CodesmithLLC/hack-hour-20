...
function matchWord(str) {
    ...
    for (let i = 0; i < newStr.length; i++) {
        if (newStr.charAt(i).match(/[^A-Za-z0-9]/g)) {
            if (newStr.charAt(i - 1).match(/[A-Za-z0-9]/g)) {
                newArr.push(newStr.charAt(i - 1))
            }
            if (newStr.charAt(i + 1).match(/[A-Za-z0-9]/g) && newArr.length !== 0) {
                newArr.push(newStr.charAt(i + 1))
            }
        }
        if (newArr.legnth === 2) {
